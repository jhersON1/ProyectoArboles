...
public class ArbolMViasBusqueda<K extends Comparable<K>,V> 
    ...
    public void insertarDatoOrdenadoEnNodo(NodoMVias<K,V> nodoAuxiliar,K claveAInsertar, V valorAInsertar ) {
        int i = orden - 1;
        while(i>0){
...
// RECORRIDO POR NIVELES ITERATIVO
    ...
    protected int posicionPorDondeBajar(NodoMVias<K, V> nodoActual, K dato) {
        for (int i = 0; i < nodoActual.cantidadDeClavesNoVacias(); i++) {
            K datoActual = nodoActual.getClave(i);
    ...
    protected void insertarDatoEnNodo(NodoMVias<K,V> nodoActual, K datoAInsertar) {
            ...
            for (int i = 0; i < cant; i++) {
                ...
                if (datoAInsertar.compareTo(datoActual) < 0) {
                    for (int k = cant; k > i; k--) {
                        K dato = nodoActual.getClave(k - 1);
                        nodoActual.setClave(k, dato);
                    }
                    nodoActual.setClave(i, datoAInsertar);
                    return;
                }
            }
            nodoActual.setClave(cant, datoAInsertar);
        }
    }
}
